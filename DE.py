...
def getArgs(time, pri, sec, T, arg_tree = False):
    ...
    for t in times:
        ...
        # M = LBX(wcnf ,use_cld=True, solver_name='m22')
        # H = Hitman(solver='m22', htype='maxsat')
        ...
        #     if seed is None:
        #         break
        #     counter = [list(wcnf.soft[s - 1]) for s in seed]
            
          
        #     if not sat(counter, arg):                
        #         # print(counter, arg_)
        #         if not any(is_subset(i, counter)  for i in pri.arguments):
        #             # counter.append(arg_)
        #             pri.commit_store_args[time].append(counter)
        #             pri.arguments.append(counter)
        #             # return counter, unsat, arg
        #             return counter
                
        #         H.block(seed)
        # print("Arg to attack: ", arg)
